      ...
      <div className="w-full max-w-sm relative z-10">
        ...
        <div className="glass rounded-2xl p-6 border border-slate-700/30">
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs text-slate-700">ou par email</span>
            <div className="flex-1 h-px bg-slate-800" />
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <Input label="Email" type="email" placeholder="vous@exemple.com" value={email} onChange={(e) => setEmail(e.target.value)} icon={<Mail size={14} />} autoComplete="email" required />
            <Input label="Mot de passe" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} icon={<Lock size={14} />} autoComplete="current-password" required />
        ...
        <p className="text-center mt-3">
          <Link href="/" className="text-xs text-slate-700 hover:text-slate-500 transition-colors">← Accueil</Link>
        </p>
      </div>
    </div>
  );
